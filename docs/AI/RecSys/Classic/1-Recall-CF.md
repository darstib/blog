...
> [!tip] 工具套件可在 [RecSys](../index.md#工具) 查看。
...
## Matrix Factorization (MF in recall)
...
![https://zhuanlan.zhihu.com/p/69662980](https://raw.githubusercontent.com/darstib/public_imgs/utool/2601/04_260104-173631.png)

> [!note]+ 
> 
> 这其实构成了用户/物品的**隐向量**，这与现在深度学习中的向量嵌入是类似的。
> 
> 实际上MF学出来的每一维不一定都能有这么好的解释，很多时候甚至可能都无法直观地解释每一维，但每一维的确又代表了一个类别，因此我们把这种类别称为隐类别，把用户向量和歌曲向量中的每一维称为隐变量。所以矩阵分解是一种隐变量模型。
...
> - [实际实践表明](https://www.bilibili.com/video/BV1b34y1e7En?t=480.0)：
> 	- 使用用户与物品有交互（曝光后点击等）的数据点作为正样本训练是正确的；但是曝光后未交互作为负样本是错误的。
> 	- 内积不如余弦相似度，平方损失不如交叉熵损失

> [!extra]-  [推荐系统之矩阵分解模型-原理篇](https://zhuanlan.zhihu.com/p/360689325)补充了一些内容
> 

可以进一步地精细地将数据分为显式数据（如评分、打星，行为置信度较高但数据量少）和隐式数据（如点击、浏览，行为置信度较低但数据量大）。

使用两种数据训练的主要区别在于：

- 使用显式数据计算模型损失时，没有用户-物品交互的单元格无需考虑
- 使用隐式数据计算模型损失时，对于为空/0 的单元格同样需要拟合
	- 同时使用隐式数据往往得到的是二值数据，考虑引入“置信度”的概念

训练模型时，使用最小交叉二乘法，即交替固定 P/Q，使用残差平方和损失优化 Q/P，这有利于增量矩阵分解。具体而言，如果增加了新的用户/物品，已经训练好的物品/用户矩阵可以不变，继续优化求解新的用户/物品向量即可。
